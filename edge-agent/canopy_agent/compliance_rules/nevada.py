...
RULES = StateComplianceRules(
    ...
    retail=RetailRules(
        ...
        recreational_purchase_limits=(
            PurchaseLimit(2.5, "ounces_flower", "per_transaction", "NRS 678B.550, read directly, effective 1/1/2024 per SB 277 (2023): \"shall not sell to a person, in any one transaction, more than 2.5 ounces of usable cannabis.\" PREVIOUSLY-FLAGGED CONFLICT NOW RESOLVED: the old NCCR 7.025 (Rev. 03/23) stale-1oz text this project used to cite is no longer the operative regulation citation — the CCB's own site (ccb.nv.gov/laws-regulations) now states the standalone NCCR document has been superseded by codification into the Nevada Administrative Code, and NAC 678B.309 ('Prohibition on sale of cannabis or cannabis products that exceed certain amounts') is the current regulatory-text home for this limit. Multiple secondary sources (search-indexed regulation summaries) converge on NAC 678B.309 now stating this same 2.5oz figure, matching the statute — but a direct fetch of NAC 678B.309's verbatim text could not be completed this pass (site access issues), so the RESOLUTION of the conflict is secondary_source-strength even though the underlying NRS 678B.550 statutory figure remains primary_source"),
            PurchaseLimit(7.1, "grams_concentrate", "per_transaction", "NRS 678B.550, read directly: \"or more than one-fourth of an ounce of concentrated cannabis\" (0.25oz ≈ 7.1g), effective 1/1/2024 per SB 277 — see the flower-limit note: the old NCCR-vs-statute conflict is now believed resolved via recodification into NAC 678B.309, but that section's exact current text wasn't independently re-fetched this pass"),
        ),
